...
Route::post('/user/creer', function (\Illuminate\Http\Request $request) {
    ...
    $role        = in_array($request->input('role'), ['admin','utilisateur']) ? $request->input('role') : 'utilisateur';
...
Route::post('/user/{id}/modifier', function (\Illuminate\Http\Request $request, $id) {
    ...
    $prenom = trim($request->input('prenom', ''));
    $email  = trim($request->input('email', ''));
    $role   = in_array($request->input('role'), ['admin','utilisateur']) ? $request->input('role') : ($target->role ?? 'utilisateur');
